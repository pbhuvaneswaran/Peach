...
import { Drawer, btnBase, btnStyle } from '../lib/motion'
...
export function ArticleEditorPanel({ open, onClose, articleId, title, initialHtml, initialMarkdown, initialQuality, angle, loading }) {
  return (
    ...
    <Drawer show={open} onClose={() => {}} backdropClassName="bg-[#172554]/40" panelClassName="w-full sm:w-[70%] h-full bg-white shadow-2xl flex flex-col">
      <div className="px-6 sm:px-10 py-4 border-b border-gray-100 shrink-0">
        <button onClick={onClose} className={`text-sm font-semibold text-gray-500 hover:text-gray-800 mb-2 ${btnBase}`} style={btnStyle()}>← Back to outline</button>
